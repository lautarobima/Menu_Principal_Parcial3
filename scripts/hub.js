...
const gamesAPI = async () => {
    const url = "https://diligent-victory-production.up.railway.app/products"
...
const setGamesDisplay = (arr, docElement, buttonAddID) => {
    docElement.innerHTML += arr.map(
        ...
        ({ id, title, releaseDate, developer, boxArt, price }) => {
            return `
...
async function startApp() {
    await gamesInCatalog();
    for (i in document.getElementsByClassName('loader'))
        document.getElementsByClassName('loader')[i].classList.add('hidden');
}
...
function selectLogInWindow(){
    let window = document.getElementById("logInWindow");
...
async function BuyRequest(){
    ...
    for (var i = 0; i<cartItems.length; i++) {
        if (items.has(cartItems[i].id)) 
            items.set(cartItems[i].id, items.get(cartItems[i].id) + 1);
        else
            items.set(cartItems[i].id, 1);
    }
    ...
    // post request
    const request = new Request("https://diligent-victory-production.up.railway.app/requests", {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
        body: JSON.stringify({name : name, phone: phone, address : address, price: totPrice, status: "pendiente"})
    });
    ...
    try {
        ...
        for (var i = 0; i<ids.length; i++) {
            const request2 = new Request("https://diligent-victory-production.up.railway.app/requests/"+responseData.id+"/products/"+ids[i], {
            method: "PUT",
            headers: {"Content-Type": "application/json"},
            body: JSON.stringify({cuantity : ids[i]+":"+items.get(ids[i])})
            });
